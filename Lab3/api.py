...
def ecc_sign_message():
    ...
    # Load khóa riêng tư để ký
    private_key, _ = ecc_cipher.load_keys()

    signature = ecc_cipher.sign(message, private_key)
    signature_hex = signature.hex() # Chuyển đổi chữ ký sang chuỗi hex để gửi qua JSON
    return jsonify({'signature': signature_hex})
...
def ecc_verify_signature():
    ...
    # Load khóa công khai để xác thực
    _, public_key = ecc_cipher.load_keys()

    # Chuyển đổi chữ ký từ hex trở lại bytes
    signature = bytes.fromhex(signature_hex)
